...
HRESULT D2DApp::CreateDeviceResources()
{
	...
	{
		RECT rc;
		WinApp& winApp = Framework::GetInstance().winApp;
		GetClientRect(winApp.hWnd, &rc);
		D2D1_SIZE_U size;
		if (!winApp.isFullScreen)
			size = D2D1::SizeU(rc.right - rc.left, rc.bottom - rc.top);
		else
			size = D2D1::SizeU(winApp.width, winApp.height);
		hr = factory->CreateHwndRenderTarget(D2D1::RenderTargetProperties(),
			D2D1::HwndRenderTargetProperties(winApp.hWnd, size), &renderTarget);
